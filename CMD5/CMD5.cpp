...
namespace CMD5 {
	...
	String^ Md5Encode::Encode(String^ src_info){
		// CLI 的String^ 类型转换为std::string 类型
		std::string c_src_info = marshal_as<std::string>(src_info);
		ParamDynamic param;
		param.ua_ = kA;
		param.ub_ = kB;
		param.uc_ = kC;
		param.ud_ = kD;
		std::string result;
		const char *src_data = c_src_info.c_str();
		...
		// 将C++的 std::string 类型转换为CLI的String^类型
